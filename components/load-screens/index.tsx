...
const LoadScreen = (props) => {
  ...
  const showTransactionStep =
    contractTransaction.step != null && contractTransaction.totalStep != null;
    ...
    <>
      <Modal isOpen={contractTransaction.modalOpen} onClose={closeModal}>
        ...
        <ModalContent
          textAlign={'center'}
          maxWidth={400}
          height={showTransactionStep ? 497 : 437}
          padding={'60px 0'}
          borderRadius={15}
        ...
        >
          ...
          <ModalBody>
            ...
            {hasAmount(contractTransaction) && (
              <Text
                ...
                color="#979797"
              >
                {showAmount(contractTransaction)}
              </Text>
            )}

            {showTransactionStep && (
              <>
                <Text marginBottom="10px" color={'#969696'}>
                  Transaction <b>{contractTransaction.step}</b> of{' '}
                  <b>{contractTransaction.totalStep}</b>
                </Text>
                <Text
                  marginBottom="10px"
                  color={'#969696'}
                  ...
                  padding="0 60px"
                >
                  Do not leave this page until confirming all transactions.
                </Text>
              </>
            )}
            <Text
              marginBottom="10px"
              fontSize={
                contractTransaction.state == REJECTED_STATUS ? '20px' : '15px'
              }
              padding="0 60px"
              fontWeight="500"
              lineHeight="18px"
              color={
                contractTransaction.state == REJECTED_STATUS
                  ? '#FF2323'
                  : '#969696'
              }
            >
              {contractTransaction.message}
            </Text>
          ...
          <ModalFooter marginTop={10} style={{ justifyContent: 'center' }}>
            {contractTransaction.button && (
              ...
              >
                {contractTransaction.button.text}
              </Badge>
            )}
          </ModalFooter>
        </ModalContent>
    ...
    <div className={styles.wrapper}>
      ...
      >
        {isCompleted(state) ? (
          ...
          <>
            <path
              className={styles.crossPathLeft}
              fill="none"
              d="M16,16 l20,20"
            />
            <path
              className={styles.crossPathRight}
              fill="none"
              d="M16,36 l20,-20"
            />
          </>
